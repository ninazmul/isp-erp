...
import ResellerInvoiceTemplate from "./ResellerInvoiceTemplate";
...
export default function ResellerInvoiceDownloader({
  bill,
}: {
  bill: ResellerBill;
}) {
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    const toastId = toast.loading("Generating Reseller PDF invoice...");
    ...
    try {
      ...
      pdf.save(`reseller_invoice_${bill.invoiceNumber}.pdf`);
      toast.success("Reseller invoice downloaded", { id: toastId });
    } catch (err) {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Reseller Invoice - ${bill.invoiceNumber}</title>
          ${styles}
          <style>
            @page {
              size: A4 portrait;
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <div className="fixed left-[-9999px] top-0 pointer-events-none">
        <div ref={invoiceRef}>
          <ResellerInvoiceTemplate bill={bill} />
        </div>
      </div>
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogTrigger asChild>
          <Button
            ...
            title="Preview Reseller Invoice"
          >
        ...
        <DialogContent className="max-w-4xl bg-slate-100 max-h-[92vh] overflow-y-auto p-0 rounded-2xl border-none">
          <div className="sticky top-0 z-30 bg-white/95 backdrop-blur-md px-6 py-3 border-b border-slate-200 flex items-center justify-between shadow-xs">
            <DialogTitle className="text-sm font-bold text-slate-800 flex items-center gap-2">
              Reseller Invoice #{bill.invoiceNumber}
            </DialogTitle>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                className="h-8 text-xs rounded-xl bg-violet-700 hover:bg-violet-800 text-white gap-1.5 shadow-sm font-bold"
              >
