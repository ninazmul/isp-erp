...
async function getNextInvoiceSequence(): Promise<number> {
    const bills = await Bill.find({ invoiceNumber: { $regex: /^INV\d+$/i } })
        .select("invoiceNumber")
        .lean<{ invoiceNumber: string }[]>();

    let maxSeq = 0;
    for (const b of bills) {
        const num = parseInt(b.invoiceNumber.replace(/^INV/i, ""), 10);
...
export async function generateMonthlyBills(month: number, year: number) {
    ...
    let invoiceSeq = await getNextInvoiceSequence();
...
export async function getBills(params?: {
    ...
    if (search.trim()) {
        ...
        const matchingCustomers = await Customer.find({
            $or: [{ name: regex }, { customerCode: regex }, { phone: regex }],
        })
            .select("_id")
            .lean<{ _id: string }[]>();
