...
export async function updatePackage(id: string, name: string, monthlyFee: number) {
    await connectToDatabase();
    const trimmedName = name.trim();
    if (!trimmedName) throw new Error("Package name is required");
    ...
    if (existing) throw new Error("Another package with this name already exists");

    const oldName = pkg.name;
    pkg.name = trimmedName;
    pkg.monthlyFee = Number(monthlyFee) || 0;
    await pkg.save();

    // If package name changed, update existing active customers with the old package name
    if (oldName !== trimmedName) {
        await Customer.updateMany(
            { packageName: oldName },
            { $set: { packageName: trimmedName } }
        );
    }
...
export async function deletePackage(id: string) {
    ...
    if (inUseCount > 0) {
        throw new Error(
            `Cannot delete: "${pkg.name}" is used by ${inUseCount} active customer(s). Reassign them first.`
        );
    }
